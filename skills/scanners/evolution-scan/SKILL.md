...
4. Write findings, validate, render the explorer, and report per the core workflow. Scanner id: `evolution-scan`, version `1.0`. For `stats`, use the script's `stats` object (it already carries `history_data: "present"`) plus a `timeline_params` key recording `depth`, `period`, and `top`.
   ...
   **Stable ids.** Era slugs are *topical*, never positional or dated: `eras/typescript-cli`, `eras/rust-workspace`, `eras/app-server-platform` — a re-run a quarter later keeps the same eras (their end dates may move) and adds a new one, which the differ then reports as new. The same rule applies to rewrites (`lifecycle/tui-to-tui2`) and people (`people/core-ownership-shift`). See the fixed-slug table under **Stable ids** below — most of this scanner's findings answer a recurring question and take a fixed slug; only eras and named migrations get a free, topical one.
...
Every re-run of this scanner on the same project must produce the same ids for the same subjects — sibling scanners reference them (`finding:evolution-scan/...`), and a renamed slug silently breaks those references. The first re-run of this scanner persisted **1 of 19 ids** because every slug was free-text; the rule below is the fix.

Most findings answer a **recurring question** and take a fixed slug regardless of the answer's wording. Only two kinds get a free slug: an **era** (named for what the project was then) and a **named migration** (`lifecycle/<old>-to-<new>`, from the code's own names).

| group | fixed slugs |
|---|---|
| `eras` | free topical slug per era (`typescript-cli`, `maintenance-plateau`), plus `turning-point-<topic>` for a singular turning point that earns its own finding |
| `growth` | `growth-curve` (the shape over time), `code-origins` (where the bulk came from — imports, generated, vendored), `survival` (how much of what was written still exists, most-rewritten files) |
| `focus-shift` | `center-of-gravity` (which area dominates now and which used to), `<area>` — the bare area name the script uses, one finding per area that deserves a verdict (`focus-shift/codeexplorer`), with the verdict (`emerging`, `cooling`, `dormant`, `accelerating`) in the **title and description**, never in the slug: an area whose verdict moves from cooling to dormant is the *same subject* and must keep its id so the differ reports a change rather than a deletion plus an addition |
| `people` | `team-shape` (size and concentration over time), `ownership` (who owns the core and whether it moved), `arrivals`, `departures`, `rising` (a dormant contributor returning at volume is `rising`, not `arrivals`) |
| `lifecycle` | `<old>-to-<new>` per migration (free, from the code's names), `oldest-code` (what survives from the start and whether it still matters), `deletions` (directories removed wholesale) |
| `trajectory` | `velocity` (the trend and the 90-day comparison), `work-mix` (feature vs fix vs refactor, now vs a year ago), `direction` (what the last period implies and the risks that follow), `no-history` (when the history is absent) |

**Naming a migration.** `lifecycle/<old>-to-<new>` takes the names from the code, not from prose: when a side is a package, module or directory, use that identifier verbatim (`statichtml-to-client-rendered`, not `server-to-client-rendering`); when both sides are named tools, use the tool names (`graphviz-to-mermaid`). If one side has no name in the code, name it for the mechanism it replaces (`iframe-to-native`), and say in the description what you named it after — that sentence is what keeps the next run consistent.

**Where measurability goes.** Whether the history is legible enough to trust (commit messages that became classifiable, a git export that starts mid-life) is not a finding of its own: state it inside `trajectory/work-mix` (for theme mix) or `growth/growth-curve` (for volume), as the caveat on the number it qualifies.
