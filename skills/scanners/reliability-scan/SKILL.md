...
- **`security-design-scan`** owns sandboxing as a *trust* boundary and the approval/fail-closed *policy*. The same process boundary is fair game here as a *failure* boundary — describe what it contains, not what it protects against. A tolerant parser it has judged for input validation is cited here only for its failure consequence, with the cross-reference.
