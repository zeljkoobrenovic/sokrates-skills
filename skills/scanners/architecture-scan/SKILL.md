...
1. Orient per the core skill. Read `config.json`'s `logicalDecompositions` — Sokrates' component split is your starting hypothesis, not your conclusion; part of this scan's value is saying where that split matches reality and where it doesn't. Prior findings (`_sokrates/reports/ai-insights/*.json`) supply the tech skeleton.
...
Follow the core workflow: write `_sokrates/reports/ai-insights/architecture-scan.json`, validate until OK, render the explorer, report leading with the style narrative (the architecture in three sentences) and any above-info findings.
