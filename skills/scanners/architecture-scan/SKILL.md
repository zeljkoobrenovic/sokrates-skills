...
- Security boundaries (trust, sandboxing) are `security-design-scan`'s; cite its findings when an architectural boundary is also a trust boundary.
...
2. **Extract the cheap structure first.** Workspace manifests are a free dependency graph: Cargo workspace members and their inter-crate `dependencies`, package.json workspaces, Bazel deps. Build the component-level directed graph from manifests before reading any code, and compute its shape (layers? hub-and-spoke? cycles?). Directory naming and README/docs architecture statements give you the *stated* design to compare against.
...
Absences are evidenced by the nearest delimiting positive fact. Strengths are findings — a boundary that has held cleanly under churn deserves an `info` entry; it is the baseline that makes the violations meaningful. Expect roughly 10–20 findings (this scanner's taxonomy is broad, so it sits at the high end of the family's usual band); merge over split, cluster minor components rather than enumerating them, and cluster minor boundary crates into one finding.
...
- `high` — erosion that actively misleads or blocks: a dependency cycle among core components, a boundary so leaky that its contract no longer constrains anything, two sources of truth for a central concept.
- `medium` — deviations that tax every change in an area: systematic skip-layer access, a hub module everything imports for unrelated reasons, duplicated domain concepts drifting apart across a boundary.
...
Use these `stats` keys where they apply (canonical, counting things in the structure, never findings — the example numbers are fictional): `components_mapped` (major components/clusters given responsibility statements), `boundaries_traced` (contracts read at the artifact level), `violations_found` (distinct deviations, not findings about them), `cycles_found` (*production* dependency cycles; report test-only cycles under an extra key like `test_only_cycles`), e.g. `{"components_mapped": 99, "boundaries_traced": 99, "violations_found": 99, "cycles_found": 99}`. Add extras freely alongside.
