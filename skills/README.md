...
| folder | purpose | when |
|---|---|---|
| [`scanners/`](scanners/README.md) | **Analysis skills** — read a finished Sokrates analysis (`_sokrates/`) and add the semantic layer: tech stack, architecture, risks, CI/CD, observability, security, domain language, evolution. Uniform findings JSON + the AI Insights Explorer. | after `sokrates analyze` |
...
| [`illustrators/`](illustrators/README.md) | **Illustrators** — optional scripts that add generated visuals to scanner results, e.g. `generate_summary_visuals.py` (one Gemini-generated illustration per scanner summary, shown in the explorer's summary block). | after the scanners |
