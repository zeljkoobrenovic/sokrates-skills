...
3. **Walk the untrusted inputs across the boundaries architecture mapped** — reference `trust-map` for the entry-point list instead of re-listing it; `input-handling/boundary-validation` describes the *validation posture* at those entries, not the entries. Operator-supplied arguments (an output folder, a CLI flag) are not untrusted input; do not invent checks for them even when a sibling's hand-off suggests it. Network responses, model output, user files and repositories, plugins/extensions, IPC, request bodies: follow the two or three most important paths from entry to the first sink and note where each is validated, constrained, or trusted. In AI systems the model-output path is untrusted input: what can it cause without a human gate?
...
Slugs are **mechanism, category or artifact names, never consequences**. Fixed slugs (use only when the subject exists). Clean verdicts: **one per group**, under the group's most relevant slug, with every swept sub-category listed in `attributes.swept` — never one finding per clean sub-category (that is padding, and on a thin target it would be ten findings saying "none"):
...
| `third-party-trust` | `plugins-and-extensions`, `updates-and-installers`, `dependencies`, `runtime-loaded-scripts` (CDN scripts in produced or served pages: SRI, pinning, what they can read), `model-output` |
...
- `critical` — a real credential in the tree; a confirmed injection path (attacker-influenceable data reaches the sink, nothing in between) **with material impact** — code execution, credential or data theft, or a served origin; a boundary whose "check" is demonstrably a no-op on a dangerous path. A confirmed injection into a static artifact with nothing to steal is `high`.
...
- `info` — the mechanism map, clean verdicts, intentional published identifiers, mitigated patterns worth documenting, the posture; an output that is sensitive *by purpose* (a report embedding the analyzed source) with documented switches to reduce it.
