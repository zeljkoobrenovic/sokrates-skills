...
- A trust boundary that coincides with an architectural boundary is described once, in `security-boundaries`, with the `boundaries` finding referencing it; a communication channel that crosses a trust boundary stays in `communication` with the boundary referenced.
