...
Fixed slugs are **used only when the subject exists** — no absence findings except `serial-stages` (a tool that never parallelized) and `missing-limits`; half the table is expected to stay empty for any given system kind. Parametrised slugs (`<stage>`, `<call>`) take the kebab-case **stage name the tool itself uses** — its stopwatch/log labels, CLI step names or pipeline enum — and only when none exist the entry method's name; so one fix changes one id. Slug rules: `linear-lookups-<stage>` when the shape is a `List.contains`/`indexOf` inside a per-item loop (the fix is a `Set`), `quadratic-<stage>` for nested iteration or pairwise work; one `regex-per-item-<stage>` finding **per stage**, never merged; a stage that is deliberately optimized *and* keeps a residual super-linear step gets the `quadratic-`/`linear-lookups-` finding with the optimizations in `attributes.mechanisms` — not an `optimized-` finding as well; `unbounded-growth` covers growth with session duration *or across runs on disk*; `missing-limits` is a roll-up allowed to repeat subjects that carry their own finding, listing them in `attributes`; `configurable-stages` exists only when `reliability-scan` has no `degradation/optional-stages` finding — otherwise reference it inside `caps-and-skips`.
...
- `hot_files_read` — files read in full (every line); `hot_files_outlined` — files where only the grep-located loop bodies were read
