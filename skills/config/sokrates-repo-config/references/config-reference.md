...
| `customTabs` | CustomTab[] | `[]` | extra tabs at the end of the report's tab strip (after **Data**), each an iframe: `{"label", "iframeLink"}` — both required, blank entries skipped; `iframeLink` absolute or relative to `reports/html/`; labels unique (trimmed, case-insensitive) (`core/CustomTab.java`, `CodeConfiguration.addOrReplaceCustomTab`) |
...
| `coAuthors` | `{enabled: true, trailerKeys: [...], aiAgents: [...]}` | how commit-message trailers become co-authors and AI-agent attribution (`analysis/CoAuthorsConfig.java`, see below) |
...
### `fileHistoryAnalysis.coAuthors` (`analysis/CoAuthorsConfig.java`, `AiAgentPattern.java`; added 2026-08-27)

`extractGitHistory` now writes two sidecars next to `git-history.txt`: `git-commits.txt` (`<sha> <first message line>`) and `git-commit-trailers.txt` (`<sha> <Key>: <value>` for every trailer, plus pseudo-trailers `Committer:` when the committer differs from the author and `Message-Signature:` for tool signature lines such as `🤖 Generated with [Claude Code](…)`). Histories extracted by older versions have no sidecars — the feature is inert until `extractGitHistory` is re-run.

| key | default | meaning |
|---|---|---|
| `enabled` | `true` | `false` switches co-author/AI analysis off (sidecar not read, no commits-explorer column, no "AI co-authored commits" row) |
| `trailerKeys` | `["Co-authored-by", "Assisted-by", "Generated-by", "Message-Signature"]` | trailer keys (case-insensitive) whose value names a co-author; `Signed-off-by`, `Committer` are ignored; a `Message-Signature` only ever resolves to an AI agent, never a person |
| `aiAgents` | Claude Code, GitHub Copilot, Cursor, OpenAI Codex, Aider, Gemini, Devin, Jules (each `{name, patterns}`) | regexes matched entirely, case-insensitively, against the whole `Name <email>` value and the email alone; first match wins; a co-author matching none but matching `bots` becomes the pseudo-agent `bot` |

A co-author that is not an agent is a person and goes through the same pipeline as authors: `ignoreContributors`, `anonymizeContributors`, `transformContributorEmails`, then `config-people.json` — so co-authored and authored commits collapse to one identity. **Setting `trailerKeys` or `aiAgents` replaces the defaults** (no merge); copy the default list and extend it. Reports: an "AI co-authored commits" row on the Overview, per-slot `aiCoAuthoredCommitsCount`, and `coauthor:`/`ai:` filters in the commits explorer.
...
- `addCustomTab -label <text> -iframeLink <url-or-relative-path> [-confFile ./_sokrates/config.json]` — appends to `customTabs`, or overwrites the tab with the same label; the AI Insights explorer is embedded with `-label "AI Insights*" -iframeLink "../ai-insights/index.html"` (relative to `reports/html/`); `generateReports` afterwards.
- `extractGitHistory` (→ `git-history.txt` + sidecars `git-commits.txt`, `git-commit-trailers.txt`), `extractGitSubHistory -prefix` (copies the sidecars into each split), `extractFiles -pattern -destFolder -destParent`, `createConventionsFile` (→ `./analysis_conventions.json`), `exportStandardConventions`, `updatePeopleConfigByUserName` (→ `_sokrates/config-people.json`).
